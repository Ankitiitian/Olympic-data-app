...
def most_successful(df, sport):
    ...
    # Get the top 15 most successful athletes by name count
    top_athletes = temp_df['Name'].value_counts().reset_index()
    top_athletes.columns = ['Name', 'count']  # Rename columns for clarity

    result = top_athletes.head(15).merge(df, left_on='Name', right_on='Name', how='left')[
...
def most_successful_country_wise(df, country):
    ...
    result = top_athletes.head(10).merge(df, left_on='Name', right_on='Name', how='left')[['Name','count','Sport']].drop_duplicates('Name')
    result.rename(columns={'count':'Medals'},inplace=True)
    return result
